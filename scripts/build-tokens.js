...
function cleanFont(v) {
  if (typeof v === 'string') {
    return v.replace(/GitLab Sans/g, 'Inter').replace(/GitLab Mono/g, 'JetBrains Mono');
  }
  if (Array.isArray(v)) return v.map(cleanFont);
  return v;
}
function lightValue(name) {
  return cleanFont(tokens[name].$value);
}
function darkValue(name) {
  const v = tokens[name];
  return cleanFont(Object.prototype.hasOwnProperty.call(v, 'dark_value') ? v.dark_value : v.$value);
}
...
function header(comment) {
  return `${comment} Pajamas-inspired design tokens (MIT)
...
function familyArray(n) {
  let v = String(lightValue(n));
  // 品牌字体不可分发：GitLab Sans -> Inter，GitLab Mono -> JetBrains Mono
  v = v.replace(/GitLab Sans/g, 'Inter').replace(/GitLab Mono/g, 'JetBrains Mono');
  return [v];
}
function jsKey(n) {
...
// ---------- 5. Tailwind ----------
{
  const conf = `/** Tailwind config generated from Pajamas-inspired tokens (MIT).
...
// ---------- 8. JS theme + d.ts ----------
{
  ...
  const js = `// Pajamas-inspired JS theme (MIT). Generated by scripts/build-tokens.js
...
// ---------- 10. YAML ----------
{
  const yaml = ['# Pajamas-inspired design tokens (MIT)', '# Generated by scripts/build-tokens.js', '', 'light:',
    'dark:'];
  ...
  write('yaml/tokens.yaml', ['# Pajamas-inspired design tokens (MIT)', '# Generated by scripts/build-tokens.js', '', 'light:', ...rows.light, '', 'dark:', ...rows.dark, ''].join('\n'));
}
...
// ---------- 14. Adobe XD colors.json ----------
{
  ...
  write('xd/colors.json', JSON.stringify({ name: 'Pajamas-inspired', colors: cols }, null, 2) + '\n');
}
